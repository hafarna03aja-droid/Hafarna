...
export const analyzeMarketTrend = async (query: string): Promise<AnalysisResult> => {
  const prompt = `
    ...
    Sintesiskan informasi dari kumpulan data simulasi yang sangat besar dari postingan media sosial, forum online, dan artikel berita.
    Identifikasi tren paling penting, keluhan konsumen, dan peluang produk potensial.
    Berikan analisis yang jelas dan terstruktur dalam format JSON. Semua output teks dalam JSON harus dalam Bahasa Indonesia.
    Kolom 'topic' dalam JSON harus merupakan kueri asli: "${query}".
  `;
